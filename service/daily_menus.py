...
def create_daily_menu(db: Session, menu_id: str, weight: float, count: int, date: date):
    same_daily_menu = db.query(DailyMenu).filter(DailyMenu.menu_id == menu_id and DailyMenu.date == date).first()
...
def get_daily_menus(db: Session, date: date):
    daily_menus_orm = db.query(DailyMenu).filter(DailyMenu.date == date)
